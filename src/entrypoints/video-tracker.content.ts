...
function isMusic(): boolean {
  ...
  if (host.includes('youtube.com')) {
    // 1. Check the Official Category ID (10 = Music)
    // This is the most reliable way as it's set at the database level.
    const playerResponse = (window as any).ytInitialPlayerResponse;
    if (playerResponse?.videoDetails?.categoryId === "10") return true;

    // 2. Check the Meta Genre (Your existing logic)
    const g = document.querySelector('meta[itemprop="genre"]');
    if (g?.getAttribute('content') === 'Music') return true;

    // 3. Check for the Music Renderer (Your existing logic)
    // This catches the newer "Official Music Video" layouts.
    if (document.querySelector('ytd-music-watch-metadata-renderer')) return true;

    // 4. Check for "Art Tracks" / Topic Channels
    // These often don't have the standard renderer but have specific metadata
    // inside the description (e.g., "Provided to YouTube by...")
    const infoRow = document.querySelector('ytd-video-secondary-info-renderer');
    if (infoRow?.innerText.includes('Auto-generated by YouTube') ||
      document.querySelector('a[href*="/channel/UC-9-kyTW8ZkZNDHQJ6FgpwQ"]')) { // Topic channels
        return true;
      }
  }
  return false;
