...
/**
 * Safely resolves the hex color palette for a given theme name or custom theme ID.
 */
export function resolveThemeColors(
  themeName: string,
  customThemes: Array<{ id: string; colors: Record<string, string> }> = [],
  fallbackPreset: string = 'dark-amber'
): Record<string, string> {
  const isCustom = themeName === 'custom' || themeName.startsWith('custom_') || themeName.startsWith('custom-');

  if (isCustom) {
    const custom = customThemes.find((t) => t.id === themeName || t.id === themeName.replace('custom_', '').replace('custom-', ''));
    if (custom) return { ...custom.colors };
  }

  const preset = THEMES[themeName] || THEMES[fallbackPreset];
  if (preset) {
    return {
