...
<script lang="ts">
  /**
   * ── Video Tab Settings ──────────────────────────────────────────────────────
   * Configures automatic logging thresholds and display rules on video hosts.
   */
  ...
  let logMusicVideos = $state(false);
...
<p class="hint">Customise watch thresholds and automated logging options for video players. All changes are saved automatically.</p>

<div class="field">
  <span class="label">Auto-Queue Criteria</span>
  <div class="thresh-row">
    <label class="thresh-opt">
      <input type="radio" name="queue-thresh-type" value="time" bind:group={queueThreshType} onchange={persist} /> Minutes watched
    </label>
    <label class="thresh-opt">
      <input type="radio" name="queue-thresh-type" value="percent" bind:group={queueThreshType} onchange={persist} /> Percentage watched
    </label>
  </div>
</div>

<div class="field">
  <label class="label" for="queue-thresh-min">Auto-Queue Limit
  ...
  {:else}
    <div class="thresh-spinner">
      ...
      <div class="thresh-spin-btns">
        ...
        <button type="button" class="thresh-spin-dn" onclick={() => { queueThreshMin = stepValue(queueThreshMin, 'down'); persist(); }} aria-label="Decrement queue threshold" title="Decrement">
          <svg viewBox="0 0 10 6" aria-hidden="true"><polyline points="1,1 5,5 9,1" /></svg>
        </button>
      </div>
    </div>
    <p class="hint" style="margin-top:7px;margin-bottom:0">Automatically place the video in your pending queue once this watch threshold has been met.</p>
  {/if}
</div>

<div class="field">
  <label class="toggle">
    <input type="checkbox" class="toggle-chk" bind:checked={autoSend} onchange={persist} />
...
<div id="auto-config" class:dim-block={!autoSend}>
  <div class="field">
    ...
    <div class="thresh-row">
      <label class="thresh-opt">
        <input type="radio" name="thresh-type" value="time" bind:group={threshType} onchange={persist} /> Minutes watched
      </label>
      <label class="thresh-opt">
        <input type="radio" name="thresh-type" value="percent" bind:group={threshType} onchange={persist} /> Percentage watched
      </label>
    </div>
  </div>
  <div class="field">
    <label class="label" for="thresh-min">Instant Submission Threshold
    ...
    {:else}
      <div class="thresh-spinner">
        ...
        <div class="thresh-spin-btns">
          ...
          <button type="button" class="thresh-spin-dn" onclick={() => { threshMin = stepValue(threshMin, 'down'); persist(); }} aria-label="Decrement send threshold" title="Decrement">
            <svg viewBox="0 0 10 6" aria-hidden="true"><polyline points="1,1 5,5 9,1" /></svg>
          </button>
        </div>
      </div>
      <p class="hint" style="margin-top:7px;margin-bottom:0">Directly send the immersion log to NihongoTracker once this accumulated watch time is reached.</p>
    {/if}
  </div>
</div>
...
<div class="field">
  <label class="toggle">
    <input type="checkbox" class="toggle-chk" bind:checked={hideButtons} onchange={persist} />
    <span class="toggle-track"><span class="toggle-thumb"></span></span>
...
<div class="field" class:dim-block={hideButtons} id="hide-jp-field">
  <label class="toggle">
    <input type="checkbox" class="toggle-chk" bind:checked={hideIfNotJp} onchange={persist} />
    <span class="toggle-track"><span class="toggle-thumb"></span></span>
...
<div class="field" class:dim-block={hideButtons} id="hide-music-field">
  <label class="toggle">
    <input type="checkbox" class="toggle-chk" bind:checked={hideMusic} onchange={persist} />
    <span class="toggle-track"><span class="toggle-thumb"></span></span>
    Do not display tracker status badge on music videos
  </label>
</div>

<div class="field" class:dim-block={hideButtons} style="display: flex; align-items: center; justify-content: space-between;">
  <label class="toggle" style="flex: 1;">
    <input type="checkbox" class="toggle-chk" bind:checked={logMusicVideos} onchange={persist} />
    <span class="toggle-track"><span class="toggle-thumb"></span></span>
    Allow queueing and logging of Japanese music videos
  </label>
  <div class="tooltip-wrap" style="cursor: help; margin-left: 8px;">
    <span style="display: inline-flex; align-items: center; justify-content: center; background: var(--color-border); color: var(--color-accent); border-radius: 50%; width: 18px; height: 18px; font-size: 11px; font-weight: bold; font-family: var(--font-mono);">i</span>
    <span class="tooltip" style="width: 250px; white-space: normal; line-height: 1.4;">Not recommended. Music videos generally contain repetitive lyrics and low text density, which are not considered high-quality active immersion.</span>
  </div>
</div>

<div class="field" style="margin-top: 24px;">
  <label class="toggle">
    <input type="checkbox" class="toggle-chk" bind:checked={enablePlaylist} onchange={persist} />
    <span class="toggle-track"><span class="toggle-thumb"></span></span>
...
<div class="field">
  <label class="toggle">
    <input type="checkbox" class="toggle-chk" bind:checked={hidePlaylistBadgeIcon} onchange={persist} />
    <span class="toggle-track"><span class="toggle-thumb"></span></span>
...
<div class="field">
  <label class="toggle">
    <input type="checkbox" class="toggle-chk" bind:checked={playlistHideNonJp} onchange={persist} />
    <span class="toggle-track"><span class="toggle-thumb"></span></span>
